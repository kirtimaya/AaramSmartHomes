...
export function ResetPasswordScreen({ supabase, onSuccess, onNavigateHome }: ResetPasswordScreenProps): React.ReactElement {
  const [done, setDone] = React.useState(false);
  const { password, setPassword, confirm, setConfirm, loading, error, submit } =
    useResetPasswordForm(supabase, () => { setDone(true); setTimeout(onSuccess, 2500); });
        ...
        <div className="text-center mb-8 space-y-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tighter text-foreground">
              Set New <span className="text-primary italic">Password</span>
            </h1>
            <p className="text-foreground/40 text-[10px] font-bold uppercase tracking-widest mt-1">
            ...
            <form onSubmit={e => { e.preventDefault(); submit(); }} className="space-y-4">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : (
                  <>Update Password<ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform ml-1" /></>
                )}
              </button>
            </form>
          )}
